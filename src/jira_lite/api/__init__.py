...
def create_api_blueprint():
    ...
    def toggle_command():
        """Enable or disable an MCP command via database"""
        try:
            data = request.get_json()
            ...
            from command_config import enable_command, disable_command
            from database import CommandConfig, DatabaseSession

            # Check if command exists and validate operation
            with DatabaseSession():
                try:
                    ...
                    # Validate operation
                    if not enabled and not config.can_disable:
                        return jsonify({
                            'error': f'Command {command_name} cannot be disabled (required command)',
                    ...
                    # Perform toggle
                    if enabled:
                        success = enable_command(command_name)
                        action = 'enabled'
                    else:
                        success = disable_command(command_name, disabled_by='web-ui')
                        action = 'disabled'

                    if not success:
                        return jsonify({'error': f'Failed to {action} command'}), 500

                    # Get updated counts
                    enabled_count = CommandConfig.select().where(CommandConfig.enabled == True).count()
                    disabled_count = CommandConfig.select().where(CommandConfig.enabled == False).count()

                    # Log to JSONL
                    log_event_to_jsonl('command_toggled', {
                    ...
                    return jsonify({
                        'message': f'Command {command_name} {action} successfully',
                        'command_name': command_name,
                        'enabled': enabled,
                        'restart_required': True,
                        'restart_command': 'Restart Claude Desktop to apply changes',
                        'category': config.category,
